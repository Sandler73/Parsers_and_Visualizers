...
class ConfigurationAnalyzer:
    ...
    def analyze_file(
        ...
        try:
            ...
            # Collect Global Protect data if applicable
            if device_type == DEVICE_TYPE_PANOS and self.parse_globalprotect:
                if hasattr(parser, 'get_globalprotect_data'):
                    ...
                    self.globalprotect_portals.extend(gp_data.get('portals', []))
                    self.globalprotect_gateways.extend(gp_data.get('gateways', []))
                    self.globalprotect_client_configs.extend(gp_data.get('client_configs', []))
                    self.log(f"Collected Global Protect data: {len(gp_data.get('portals', []))} portals, "
                            f"{len(gp_data.get('gateways', []))} gateways")
            ...
            if not matched_flows:
                for flow in flow_mappings:
                    if flow.ip_address:
                        ...
                        self.log(f"  FALLBACK: Matched endpoint {endpoint.name} ({endpoint.ip_address or 'no-ip'}) to interface {flow.interface_name}", level='WARNING')
    ...
    def deduplicate_endpoints(
        ...
        Rules:
        1. Host endpoints with same IP â†’ Merge into one with combined sources
        2. Network endpoints representing actual interface subnets â†’ Keep all (legitimate)
        3. Network endpoints from ACLs/object-groups with same IP â†’ Merge
